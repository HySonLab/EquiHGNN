# from https://github.com/pyg-team/pytorch_geometric/blob/master/torch_geometric/datasets/md17.py
...
class MD17Base(InMemoryDataset):
...
@registry.register_data("md17_g")
@registry.register_data("md17_g_3d")
class MD17Graph(MD17Base):
    @property
    def processed_file_names(self) -> List[str]:
        if self.ccsd:
            return ["g_train.pt", "g_test.pt"]
        else:
            return ["g_data.pt"]
    ...
    def process(self) -> None:
        ...
        for raw_path, processed_path in it:
            ...
            if self.revised:
                ...
                positions = torch.from_numpy(raw_data["coords"]).float()
                energies = torch.from_numpy(raw_data["energies"]).float()
                # forces = torch.from_numpy(raw_data['forces']).float()
            else:
...
@registry.register_data("md17_hg")
@registry.register_data("md17_hg_3d")
class MD17HGraph(MD17Base):
    @property
    def processed_file_names(self) -> List[str]:
        if self.ccsd:
            return ["hg_train.pt", "hg_test.pt"]
        else:
            return ["hg_data.pt"]
    ...
    def process(self) -> None:
        ...
        for raw_path, processed_path in it:
            ...
            if self.revised:
                ...
                positions = torch.from_numpy(raw_data["coords"]).float()
                energies = torch.from_numpy(raw_data["energies"]).float()
                # forces = torch.from_numpy(raw_data['ss']).float()
            else:
                z = torch.from_numpy(raw_data["z"]).long()
                positions = torch.from_numpy(raw_data["R"]).float()
                energies = torch.from_numpy(raw_data["E"]).float()
