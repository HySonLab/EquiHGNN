...
from equihgnn.models.layers.fa_former_layer import FAFormer, FAFormerConfig
from equihgnn.models.layers.mlp import MLP


@registry.register_model("faformer_equihnn")
class FAFormerEquiHNN(nn.Module):
    def __init__(self, num_target, args):
        ...
        self.fa_former = FAFormer(
            FAFormerConfig(
                d_input=args.MLP_hidden,
                n_layers=2,
                n_neighbors=16,
                n_heads=2,
                d_model=args.MLP_hidden,
                d_edge_model=args.MLP_hidden,
                norm="layer",
                valid_radius=5.0,
                activation="swiglu",
                n_pos=None,
    ...
    def forward(self, data):
        ...
        x, _ = self.fa_former(x.unsqueeze(0), data.pos.unsqueeze(0))
        x = x.squeeze(0)
...
@registry.register_model("faformer_equihnns")
class FAFormerEquiHNNS(nn.Module):
    def __init__(self, num_target, args):
        """Equivariant Molecular Hypergraph Neural Network,
        (Shared parameters between all message passing layers)
...
@registry.register_model("faformer_equihnnm")
class FAFormerEquiHNNM(nn.Module):
    def __init__(self, num_target, args):
