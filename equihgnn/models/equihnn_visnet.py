...
from equihgnn.models.layers.visnet_layer import ViSNet


@registry.register_model("visnet_equihnn")
class VisNetEquiHNN(nn.Module):
    def __init__(self, num_target, args):
        ...
        self.visnet_layer = ViSNet(
            hidden_channels=args.MLP_hidden, lmax=2, max_num_neighbors=16
        )
    ...
    def forward(self, data):
        ...
        x = self.visnet_layer(data.x, data.pos, data.batch)
        x = x.squeeze(0)
...
@registry.register_model("visnet_equihnns")
class VisNetEquiHNNS(nn.Module):
    def __init__(self, num_target, args):
        """Equivariant Molecular Hypergraph Neural Network,
        (Shared parameters between all message passing layers)
...
@registry.register_model("visnet_equihnnm")
class VisNetEquiHNNM(nn.Module):
    def __init__(self, num_target, args):
