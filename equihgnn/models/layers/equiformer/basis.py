...
from equihgnn.models.layers.equiformer.irr_repr import irr_repr, rot_to_euler_angles
...
def basis_transformation_Q_J(J, order_in, order_out, random_angles=RANDOM_ANGLES):
    ...
    """
    sylvester_submatrices = sylvester_submatrix(
        order_out, order_in, J, *random_angles.unbind(dim=-1)
    )
    ...
    Q_J = rearrange(Q_J, "(oi m) -> oi m", m=to_order(J))
...
def get_basis(max_degree):
    ...
    for d_in, d_out in product(range(max_degree + 1), range(max_degree + 1)):
        ...
        slice_in, slice_out = map(
            lambda t: slice_for_centering_y_to_x(t, m_min), (m_in, m_out)
        )
        ...
        for J in range(abs(d_in - d_out), d_in + d_out + 1):
            ...
            K_J = reduce(
                K_J, "o i -> i", "sum"
            )  # the matrix is a sparse diagonal, but flipped depending on whether J is even or odd
...
def rot_x_to_y_direction(x, y, eps=1e-6):
    ...
    This formulation, although not the shortest path, has the benefit of rotation matrix being symmetric;
    rotating back to x upon two rotations
    """
...
def get_D_to_from_z_axis(r_ij, max_degree):
    ...
    z_axis = r_ij.new_tensor([0.0, 1.0, 0.0])
