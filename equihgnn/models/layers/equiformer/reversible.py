...
from equihgnn.models.layers.equiformer.utils import to_order
...
class _ReversibleFunction(Function):
    ...
    def forward(ctx, x, blocks, kwargs):
        ...
        x = {
            k: rearrange(v, "... (d m) -> ... d m", m=to_order(k) * 2)
            for k, v in x.items()
        }
...
class SequentialSequence(Module):
    @beartype
    def __init__(self, blocks: List[Tuple[Module, Module]]):
        super().__init__()
...
class ReversibleSequence(Module):
    ...
    def forward(self, x, **kwargs):
        ...
        x = {
            k: reduce(v, "... (d r m) -> ... d m", "mean", r=2, m=to_order(k))
            for k, v in x.items()
        }
