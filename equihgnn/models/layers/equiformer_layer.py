# from # from https://github.com/lucidrains/equiformer-pytorch
...
from equihgnn.models.layers.equiformer.basis import get_basis, get_D_to_from_z_axis
from equihgnn.models.layers.equiformer.reversible import (
    ReversibleSequence,
    SequentialSequence,
)
from equihgnn.models.layers.equiformer.utils import (
    cast_tuple,
    default,
    exists,
    masked_mean,
...
@beartype
def fiber_product(fiber_in: Tuple[int, ...], fiber_out: Tuple[int, ...]):
    fiber_in, fiber_out = tuple(
        map(
            lambda t: [(degree, dim) for degree, dim in enumerate(t)],
            (fiber_in, fiber_out),
        )
    )
...
@beartype
def fiber_and(fiber_in: Tuple[int, ...], fiber_out: Tuple[int, ...]):
...
def split_num_into_groups(num, groups):
    ...
    return (
        *((num_per_group,) * remainder),
        *((((num_per_group - 1),) * (groups - remainder))),
    )
...
class Linear(nn.Module):
    @beartype
    def __init__(self, fiber_in: Tuple[int, ...], fiber_out: Tuple[int, ...]):
        super().__init__()
        ...
        for (degree, dim_in, dim_out) in fiber_and(fiber_in, fiber_out):
            self.weights.append(
                nn.Parameter(torch.randn(dim_in, dim_out) / sqrt(dim_in))
            )
...
class Norm(nn.Module):
    ...
    def __init__(
        ...
        """
        deviates from the paper slightly, will use rmsnorm throughout
        (no mean centering or bias, even for type0 features)
...
class Gate(nn.Module):
    @beartype
    def __init__(self, fiber: Tuple[int, ...]):
        super().__init__()

        type0_dim = fiber[0]
        dim_gate = sum(fiber[1:])

        assert type0_dim > dim_gate
...
class DTP(nn.Module):
    ...
    def __init__(
        ...
        for degree_out, dim_out in enumerate(self.fiber_out):
            ...
            split_dim_out = split_num_into_groups(
                dim_out, num_degrees_in
            )  # returns a tuple of ints representing how many channels come from each input degree

            for degree_in, (dim_in, dim_out_from_degree_in) in enumerate(
                zip(self.fiber_in, split_dim_out)
            ):
                min(degree_out, degree_in)

                self.kernel_unary[f"({degree_in},{degree_out})"] = Radial(
                    degree_in,
                    dim_in,
                    degree_out,
                    dim_out_from_degree_in,
    ...
    def forward(
        ...
        for degree_out, _ in enumerate(self.fiber_out):
            ...
            for degree_in, _ in enumerate(self.fiber_in):
                ...
                if not exists(B):  # degree_in or degree_out is 0
                    ...
                    x = repeat(
                        x, "... mi -> ... mi mf r", mf=(B.shape[-1] + 1) // 2, r=2
                    )  # mf + 1, so that mf can be divided in 2
                    x, x_to_flip = x.unbind(dim=-1)

                    x_flipped = torch.flip(
                        x_to_flip, dims=(-2,)
                    )  # flip on the mi axis, as the basis alternates between diagonal and flipped diagonal across mf
                    x = torch.stack((x, x_flipped), dim=-1)
                    x = rearrange(x, "... mf r -> ... (mf r)", r=2)
                    x = x[..., :-1]

                    output_chunk = opt_einsum(
                        "... o i, m f, ... i m f -> ... o m", R, B, x
                    )

                # in the case that degree_out < degree_in

                output_chunk = F.pad(
                    output_chunk, (maybe_output_pad, maybe_output_pad), value=0.0
                )
        ...
        self_interact_out = {
            k: rearrange(v, "... d m -> ... 1 d m")
            for k, v in self_interact_out.items()
        }
        outputs = {
            degree: torch.cat(tensors, dim=-3)
            for degree, tensors in enumerate(
                zip(self_interact_out.values(), outputs.values())
            )
        }
        return outputs
...
class Radial(nn.Module):
    def __init__(
        self, degree_in, nc_in, degree_out, nc_out, edge_dim=0, radial_hidden_dim=64
    ):
        super().__init__()
...
class L2DistAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.has_linear_attn:
            ...
            self.linear_attn = TaylorSeriesLinearAttn(
                ...
                heads=num_linear_attn_heads,
                combine_heads=False,
                gate_value_heads=True,
            )
            hidden_fiber = tuple_set_at_index(
                hidden_fiber,
                0,
                hidden_fiber[0] + linear_attn_dim_head * num_linear_attn_heads,
            )
    ...
    @beartype
    def forward(self, features, edge_info: EdgeInfo, rel_dist, basis, D, mask=None):
        ...
        keyvalues = self.to_kv(
            features, edge_info=edge_info, rel_dist=rel_dist, basis=basis, D=D
        )
        ...
        for degree, gate, h, scale in zip(
            features.keys(), gates, self.heads, self.scale
        ):
            ...
            if not one_head_kv:
                kv = rearrange(kv, "b i j (h d) m -> b h i j d m", h=h)
...
class MLPAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        value_gate_fiber = tuple_set_at_index(
            value_hidden_fiber, 0, type0_dim + htype_dims
        )

        attn_hidden_dims = tuple(head * attn_hidden_dim_mult for head in heads)

        intermediate_fiber = tuple_set_at_index(
            value_hidden_fiber, 0, sum(attn_hidden_dims) + type0_dim + htype_dims
        )
        self.intermediate_type0_split = [*attn_hidden_dims, type0_dim + htype_dims]

        # main branch tensor product

        self.to_attn_and_v = DTP(
            fiber,
            intermediate_fiber,
            radial_hidden_dim=radial_hidden_dim,
            edge_dim=edge_dim,
        ...
        self.to_values = nn.Sequential(
            Gate(value_gate_fiber), Linear(value_hidden_fiber, value_hidden_fiber)
        )
    ...
    def forward(self, features, edge_info: EdgeInfo, rel_dist, basis, D, mask=None):
        ...
        *attn_branch_type0, value_branch_type0 = intermediate[0].split(
            self.intermediate_type0_split, dim=-2
        )
        ...
        for fn, attn_intermediate, scale in zip(
            self.to_attn_logits, attn_branch_type0, self.scale
        ):
            ...
            if exists(neighbor_mask):
                attn_logits = attn_logits.masked_fill(
                    ~neighbor_mask, -torch.finfo(attn_logits.dtype).max
                )
        ...
        for degree, (attn, value, gate, h) in enumerate(
            zip(attentions, values.values(), gates, self.heads)
        ):
...
class Equiformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.token_emb = (
            nn.Embedding(num_tokens, type0_feat_dim) if exists(num_tokens) else None
        )
        ...
        self.pos_emb = (
            nn.Embedding(num_positions, type0_feat_dim)
            if exists(num_positions)
            else None
        )
        ...
        ), "edge dimension (edge_dim) must be supplied if equiformer is to have edge tokens"
        ...
        assert (
            not exists(num_adj_degrees_embed) or num_adj_degrees_embed >= 1
        ), "number of adjacent degrees to embed must be 1 or greater"

        self.num_adj_degrees_embed = num_adj_degrees_embed
        self.adj_emb = (
            nn.Embedding(num_adj_degrees_embed + 1, adj_dim)
            if exists(num_adj_degrees_embed) and adj_dim > 0
            else None
        )

        edge_dim = (edge_dim if self.has_edges else 0) + (
            adj_dim if exists(self.adj_emb) else 0
        )
        ...
        self.ff_out = (
            proj_out_klass(self.dim, (1,) * self.num_degrees)
            if reduce_dim_out
            else None
        )
    ...
    def forward(
        ...
        if exists(self.pos_emb):
            ...
            assert (
                seq_len <= self.num_positions
            ), "feature sequence length must be less than the number of positions given at init"

            feats = feats + self.pos_emb(torch.arange(seq_len, device=device))

        feats = (
            self.embedding_grad_frac * feats
            + (1 - self.embedding_grad_frac) * feats.detach()
        )

        assert not (
            self.has_edges and not exists(edges)
        ...
        assert (
            d == self.type0_feat_dim
        ), f"feature dimension {d} must be equal to dimension given at init {self.type0_feat_dim}"
        assert set(map(int, inputs.keys())) == set(
            range(self.input_degrees)
        ), f"input must have {self.input_degrees} degree"

        num_degrees, neighbors, max_sparse_neighbors, valid_radius = (
            self.num_degrees,
            self.num_neighbors,
            self.max_sparse_neighbors,
            self.valid_radius,
        )
        ...
        exclude_self_mask = rearrange(
            ~torch.eye(n, dtype=torch.bool, device=device), "i j -> 1 i j"
        )
        ...
        if self.attend_sparse_neighbors:
            ...
            adj_mat_max_neighbors = (
                reduce(adj_mat_values, "... i j -> ... i", "sum").amax().item()
            )
        ...
        rel_pos = rearrange(coors, "b n d -> b n 1 d") - rearrange(
            coors, "b n d -> b 1 n d"
        )
        ...
        max_value = get_max_value(
            modified_rel_dist
        )  # for masking out nodes from being considered as neighbors
        ...
        if exists(sparse_neighbor_mask):
            modified_rel_dist = modified_rel_dist.masked_fill(sparse_neighbor_mask, 0.0)
        ...
        dist_values, nearest_indices = modified_rel_dist.topk(
        ...
        neighbor_rel_pos = get_at(
            "b i [j] c, b i k -> b i k c", rel_pos, nearest_indices
        )
        ...
        x = self.tp_in(
            x, edge_info=edge_info, rel_dist=neighbor_rel_dist, basis=self.basis, D=D
        )
        ...
        type0 = rearrange(
            type0, "... 1 -> ..."
        )  # for type 0, just squeeze out the last dimension
