# from https://github.com/lucidrains/se3-transformer-pytorch
...
from equihgnn.models.layers.se3_transformer.basis import get_basis
from equihgnn.models.layers.se3_transformer.reversible import (
    ReversibleSequence,
    SequentialSequence,
)
from equihgnn.models.layers.se3_transformer.rotary import (
    SinusoidalEmbeddings,
    apply_rotary_pos_emb,
)
from equihgnn.models.layers.se3_transformer.utils import (
    ...
    def __init__(self, fiber_in, fiber_out):
        ...
        for (degree, dim_in, dim_out) in fiber_in & fiber_out:
            ...
            self.weights[key] = nn.Parameter(
                torch.randn(dim_in, dim_out) / sqrt(dim_in)
            )
            ...
            self.transform[str(degree)] = nn.ParameterDict(
                {
                    ...
                    "w_gate": nn.Parameter(rand_uniform((chan, chan), -1e-3, 1e-3))
                    if gated_scale
                    else None,
                }
            )
...
class ConvSE3(nn.Module):
    ...
    def __init__(
        ...
        for (di, mi), (do, mo) in self.fiber_in * self.fiber_out:
            self.kernel_unary[f"({di},{do})"] = PairwiseConv(
                di, mi, do, mo, edge_dim=edge_dim, splits=splits
            )
    ...
    def forward(self, inp, edge_info, rel_dist=None, basis=None):
        ...
        if self.fourier_encode_dist:
            rel_dist = fourier_encode(
                rel_dist[..., None], num_encodings=self.num_fourier_features
            )

        # split basis

        basis_keys = basis.keys()
        split_basis_values = list(
            zip(*list(map(lambda t: fast_split(t, splits, dim=1), basis.values())))
        )
        ...
        for degree_out in self.fiber_out.degrees:
            ...
            for degree_in, m_in in self.fiber_in:
                ...
                edge_features = (
                    torch.cat((rel_dist, edges), dim=-1) if exists(edges) else rel_dist
                )
                ...
                for x_chunk, edge_features, basis in zip(
                    split_x, split_edge_features, split_basis
                ):
...
class AttentionSE3(nn.Module):
    def __init__(
        ...
        project_out = not (
            heads == 1 and len(fiber.dims) == 1 and dim_head == fiber.dims[0]
        )
        ...
        assert not (
            linear_proj_keys and tie_key_values
        ), "you cannot do linear projection of keys and have shared key / values turned on at the same time"
    ...
    def forward(
        ...
        if self.linear_proj_keys:
            ...
            keys = map_values(
                lambda val: batched_index_select(val, neighbor_indices, dim=1), keys
            )
        ...
        if exists(global_feats):
            global_keys, global_values = self.to_global_k(
                global_feats
            ), self.to_global_v(global_feats)
        ...
        for degree in features.keys():
            ...
            k, v = map(
                lambda t: rearrange(t, "b i j (h d) m -> b h i j d m", h=h), (k, v)
            )
            ...
            if self.use_null_kv:
                null_k, null_v = map(
                    lambda t: t[degree], (self.null_keys, self.null_values)
                )
                null_k, null_v = map(
                    lambda t: repeat(
                        t, "h d m -> b h i () d m", b=q.shape[0], i=q.shape[2]
                    ),
                    (null_k, null_v),
                )
            ...
            if exists(global_feats) and degree == "0":
                global_k, global_v = map(
                    lambda t: t[degree], (global_keys, global_values)
                )
                global_k, global_v = map(
                    lambda t: repeat(
                        t, "b j (h d) m -> b h i j d m", h=h, i=k.shape[2]
                    ),
                    (global_k, global_v),
                )
    ...
    def forward(
        ...
        for degree in features.keys():
            ...
            if attend_self:
                self_k, self_v = map(lambda t: t[degree], (self_keys, self_values))
                self_k, self_v = map(
                    lambda t: rearrange(t, "b n d m -> b n () d m"), (self_k, self_v)
                )
            ...
            if self.use_null_kv:
                ...
                null_k, null_v = map(
                    lambda t: repeat(
                        t, "d m -> b i () d m", b=q.shape[0], i=q.shape[2]
                    ),
                    (null_k, null_v),
                )
            ...
            if exists(global_feats) and degree == "0":
                ...
                global_k, global_v = map(
                    lambda t: repeat(t, "b j d m -> b i j d m", i=k.shape[1]),
                    (global_k, global_v),
                )
...
class AttentionBlockSE3(nn.Module):
    ...
    def forward(
        ...
        outputs = self.attn(
            outputs, edge_info, rel_dist, basis, global_feats, pos_emb, mask
        )
...
class EGNN(nn.Module):
    ...
    def forward(self, features, edge_info, rel_dist, mask=None, **kwargs):
        ...
        for degree, htype in htypes:
            rel_htype = rearrange(htype, "b i d m -> b i () d m") - rearrange(
                htype, "b j d m -> b () j d m"
            )
        ...
        neighbor_higher_type_dists = map(
            lambda t: batched_index_select(t, neighbor_indices, dim=2), rel_htypes_dists
        )
        coor_rel_dist = rearrange(rel_dist, "b i j -> b i j ()")

        edge_mlp_inputs = broadcat(
            (nodes_i, nodes_j, *neighbor_higher_type_dists, coor_rel_dist), dim=-1
        )
        ...
        if exists(mask):
            ...
            htype_weights = htype_weights.masked_fill(~htype_mask, 0.0)

        for degree, rel_htype, htype_weight in zip(
            htype_degrees, rel_htypes, split_htype_weights
        ):
            normed_rel_htype = self.htype_norms[str(degree)](rel_htype)
            normed_rel_htype = batched_index_select(
                normed_rel_htype, neighbor_indices, dim=2
            )

            htype_update = einsum(
                "b i j d m, b i j d -> b i d m", normed_rel_htype, htype_weight
            )
...
class EGnnNetwork(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        rel_dist = F.pad(rel_dist, (1, 0), value=0.0)

        if exists(edges):
            edges = F.pad(
                edges, (0, 0, 1, 0), value=0.0
            )  # make edge of token to itself 0 for now
...
class SE3Transformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.num_degrees = (
            num_degrees if exists(num_degrees) else (max(hidden_fiber_dict.keys()) + 1)
        )
        ...
        assert exists(num_degrees) or exists(
            hidden_fiber_dict
        ...
        assert not (
            causal and not attend_self
        ), "attending to self must be turned on if in autoregressive mode (for the first token)"
        self.causal = causal
        ...
        self.accept_global_feats = exists(global_feats_dim)
        assert not (
            reversible and self.accept_global_feats
        ...
        default_attention_klass = (
            OneHeadedKVAttentionSE3 if one_headed_key_values else AttentionSE3
        )

        if use_egnn:
            self.net = EGnnNetwork(
                fiber=fiber_hidden,
                depth=depth,
                edge_dim=edge_dim,
                hidden_dim=egnn_hidden_dim,
                coor_weights_clamp_value=egnn_weights_clamp_value,
                feedforward=egnn_feedforward,
            )
        else:
                    ...
                    nn.ModuleList(
                        [
                            AttentionBlockSE3(
                                ...
                                global_feats_dim=global_feats_dim,
                                linear_proj_keys=linear_proj_keys,
                                attention_klass=attention_klass,
                                tie_key_values=tie_key_values,
                                norm_gated_scale=norm_gated_scale,
                            ),
                            FeedForwardBlockSE3(
                                fiber_hidden, norm_gated_scale=norm_gated_scale
                            ),
                        ]
                    )
                )
        ...
        self.conv_out = (
            ConvSE3(fiber_hidden, fiber_out, **conv_kwargs)
            if exists(fiber_out)
            else None
        )

        self.norm = (
            NormSE3(fiber_out, gated_scale=norm_gated_scale, nonlin=nn.Identity())
            if (norm_out or reversible) and exists(fiber_out)
            else nn.Identity()
        )
    ...
    def forward(
        ...
        if exists(self.pos_emb):
            assert (
                feats.shape[1] <= self.num_positions
        ...
        assert (
            d == self.dim_in[0]
        ), f"feature dimension {d} must be equal to dimension given at init {self.dim_in[0]}"
        ...
        if exists(self.num_adj_degrees):
            ...
            adj_indices = adj_indices.masked_select(exclude_self_mask).reshape(
                b, n, n - 1
            )
        ...
        if exists(neighbor_mask):
            ...
            if max_neighbors > neighbors:
                print(
                    f"neighbor_mask shows maximum number of neighbors as {max_neighbors}"
                    "but specified number of neighbors is {neighbors}"
                )
        ...
        if self.causal:
            ...
            modified_rel_dist = modified_rel_dist.masked_fill(
                causal_mask[None, ...], max_value
            )
        ...
        if self.rotary_position:
            ...
            neighbor_indices_with_self = torch.cat(
                (self_indices, neighbor_indices), dim=2
            )
            pos_emb = batched_index_select(
                seq_pos_emb, neighbor_indices_with_self, dim=0
            )
        ...
        if self.rotary_rel_dist:
            neighbor_rel_dist_with_self = (
                F.pad(neighbor_rel_dist, (1, 0), value=0) * 1e2
            )
        ...
        basis = get_basis(
            neighbor_rel_pos, num_degrees - 1, differentiable=self.differentiable_coors
        )
        ...
        if return_pooled:
            mask_fn = (
                (lambda t: masked_mean(t, _mask, dim=1))
                if exists(_mask)
