...
# Modified from torch_geometric.nn.VisNet
...
class VecLayerNorm(torch.nn.Module):
    ...
    def forward(self, vec: Tensor) -> Tensor:
        ...
        raise ValueError(
            f"'{self.__class__.__name__}' only support 3 or 8 "
...
class NeighborEmbedding(MessagePassing):
    ...
    def __init__(
        ...
        self.embedding = AtomEncoder(emb_dim=hidden_channels)
...
class ViS_MP(MessagePassing):
    ...
    def forward(
        ...
        x, vec_out = self.propagate(
            edge_index, q=q, k=k, v=v, dk=dk, dv=dv, vec=vec, r_ij=r_ij, d_ij=d_ij
        )
    ...
    def edge_update(
        self, vec_i: Tensor, vec_j: Tensor, d_ij: Tensor, f_ij: Tensor
    ) -> Tensor:
